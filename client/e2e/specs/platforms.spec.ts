...
test.describe('Platforms', () => {
  test('platform manager page loads', async ({ page }) => {
    await mockApi(page);
  ...
  test('sidebar has platforms link', async ({ page }) => {
    await mockApi(page);
    ...
    await expect(page.getByRole('link', { name: /broadcasting targets/i })).toBeVisible();
  });
});
