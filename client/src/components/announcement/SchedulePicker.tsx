...
export default function SchedulePicker({ scheduleDateTime, setScheduleDateTime, show, onToggle }: SchedulePickerProps) {
  if (!show) {
    return (
  ...
  return (
    <div className="border border-accent-violet/30 rounded-sm p-4 bg-accent-violet/5 space-y-3">
      ...
      <input type="datetime-local" value={scheduleDateTime} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setScheduleDateTime(e.target.value)}
