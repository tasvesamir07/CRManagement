...
interface CompileMessageParams {
...
const groupNotes = (notes: (string | NoteItem)[]): GroupedNotes => notes.reduce<GroupedNotes>((acc, item) => {
  const isObject = typeof item === 'object' && item !== null;
  const text = isObject ? (item as NoteItem).text : (item as string);
  const type = isObject ? (item as NoteItem).type : 'note';
...
export const compileMessage = ({
  ...
  notes, closingText
}: CompileMessageParams): string => {
  const appendSectionTime = (result: string, sec: Section): string => {
    let r = result;
  ...
  const appendNotesGroup = (result: string, groupedNotes: GroupedNotes): string => {
    let r = result;
  ...
  if (category === 'class_cancel') {
    ...
    const eventDate = selectedDate
      ? new Date(parseInt(selectedDate.split('-')[0]), parseInt(selectedDate.split('-')[1]) - 1, parseInt(selectedDate.split('-')[2]))
      : new Date();
  ...
  if (selectedDate) {
    const eventDate = new Date(parseInt(selectedDate.split('-')[0]), parseInt(selectedDate.split('-')[1]) - 1, parseInt(selectedDate.split('-')[2]));
    const day = String(eventDate.getDate()).padStart(2, '0');
    const month = String(eventDate.getMonth() + 1).padStart(2, '0');
    const year = String(eventDate.getFullYear()).substring(2);
    const dayName = eventDate.toLocaleDateString('en-US', { weekday: 'long' });
