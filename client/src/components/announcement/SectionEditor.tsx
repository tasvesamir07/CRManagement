...
export default function SectionEditor({ sections, setSections }: SectionEditorProps) {
  const addSection = () => setSections(prev => [...prev, { name: '', startTime: '', endTime: '', room: '', mode: 'Offline' as const, timeOption: 'select' as const }]);
  const removeSection = (i: number) => { if (sections.length > 1) setSections(prev => prev.filter((_, idx) => idx !== i)); };
  const updateSection = (i: number, field: keyof Section, val: string | boolean | undefined) => setSections(prev => { const u = [...prev]; u[i] = { ...u[i], [field]: val }; return u; });
    ...
    <div className="space-y-3">
      ...
      {sections.map((sec, i) => (
        <div key={i} className="flex flex-wrap items-center gap-2 p-3 border border-hairline rounded-sm">
          <input type="text" placeholder="Section" value={sec.name} onChange={(e: React.ChangeEvent<HTMLInputElement>) => updateSection(i, 'name', e.target.value)}
          ...
          <select value={sec.timeOption || 'select'} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => {
            const opt = e.target.value as Section['timeOption'];
          ...
          {sec.timeOption === 'custom' ? (
            <input type="text" placeholder="Custom time text" value={sec.startTime} onChange={(e: React.ChangeEvent<HTMLInputElement>) => updateSection(i, 'startTime', e.target.value)}
            ...
            <>
              <input type="time" value={sec.startTime} onChange={(e: React.ChangeEvent<HTMLInputElement>) => updateSection(i, 'startTime', e.target.value)}
              ...
              <input type="time" value={sec.endTime} onChange={(e: React.ChangeEvent<HTMLInputElement>) => updateSection(i, 'endTime', e.target.value)}
                className="px-2 py-1.5 text-xs border border-hairline rounded-sm bg-canvas text-ink focus:outline-none focus:border-primary w-[100px]" />
            </>
          ) : (
          ...
          <input type="text" placeholder="Room" value={sec.room} onChange={(e: React.ChangeEvent<HTMLInputElement>) => updateSection(i, 'room', e.target.value)}
            className="flex-1 min-w-[80px] px-2 py-1.5 text-xs border border-hairline rounded-sm bg-canvas text-ink focus:outline-none focus:border-primary" />
          <select value={sec.mode} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => updateSection(i, 'mode', e.target.value)}
            className="px-2 py-1.5 text-xs border border-hairline rounded-sm bg-canvas text-ink focus:outline-none focus:border-primary">
