...
const AnnouncementForm = () => {
  const {
    ...
    handleTemplateApply,
    handleGenerateAIDraft, getCompiledMessage,
        ...
        <form onSubmit={(e) => { e.preventDefault(); handleConfirmBroadcast(); }} className="lg:col-span-7 bg-canvas border border-hairline rounded-lg p-6 shadow-sm space-y-6">
          ...
          {broadcastMode === 'share_file' && (
            <div>
              ...
              <textarea value={fileCaption} onChange={(e) => setFileCaption(e.target.value)} placeholder="Type an optional message to accompany the file(s)..." rows={4}
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" required value={notices[0]?.title || ''} onChange={(e) => handleTitleChange(0, e.target.value)} placeholder="e.g. Makeup Class Announcement"
                ...
                <div>
                  ...
                  <div className="custom-select-wrapper">
                    <select value={notices[0]?.selectedCourseId || ''} onChange={(e) => handleCourseChange(0, e.target.value)}
              ...
              <div>
                ...
                <textarea value={customText} onChange={(e) => setCustomText(e.target.value)} placeholder="Write your notice text here... Use *bold* for emphasis." rows={8}
          ...
          {broadcastMode === 'notice' && (
            <div className="space-y-6">
                  ...
                  <div key={notice.id || nIdx} className="border border-hairline rounded-md bg-canvas shadow-sm overflow-hidden transition-all">
                    <div onClick={() => handleNoticeFieldChange(nIdx, 'isExpanded', !notice.isExpanded)}
                      ...
                      <div className="flex items-center gap-3" onClick={(e) => e.stopPropagation()}>
                        {notices.length > 1 && (
                          <button type="button" onClick={() => { if (window.confirm(`Are you sure you want to delete Notice #${nIdx + 1}?`)) { setNotices(prev => prev.filter((_, idx) => idx !== nIdx)); toast.success(`Notice #${nIdx + 1} deleted`); } }}
                      ...
                      <div className="p-4 space-y-5">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div>
                            ...
                            <div className="custom-select-wrapper">
                              <select value={notice.titlePreset} onChange={(e) => handlePresetChange(nIdx, e.target.value)}
                                className="custom-select block w-full pl-3 pr-10 h-9 py-1.5 border border-hairline rounded-sm bg-canvas focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary text-sm text-ink hover:border-hairline-strong transition-all duration-150">
                          ...
                          <div>
                            ...
                            <input type="text" value={notice.title} onChange={(e) => handleTitleChange(nIdx, e.target.value)} placeholder="e.g. Quiz - 4"
                              className="appearance-none block w-full h-9 px-3 py-1.5 border border-hairline rounded-sm shadow-sm placeholder-ink-faint focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary text-sm text-ink hover:border-hairline-strong transition-all duration-150" />
                          </div>
                        </div>
                        ...
                        <div className="bg-canvas-soft border border-hairline rounded-sm p-4 space-y-5">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className={(notice.category === 'syllabus' || notice.category === 'suggestion') ? "md:col-span-2" : ""}>
                              ...
                              <div className="custom-select-wrapper">
                                <select value={notice.selectedCourseId} onChange={(e) => handleCourseChange(nIdx, e.target.value)}
                              ...
                              {notice.sections.map((sec, idx) => (
                                <div key={idx} className="flex flex-col md:flex-row items-end gap-3 p-3 bg-canvas border border-hairline rounded-sm relative">
                                  ...
                                  <div className="w-full md:w-[10%]">
                                    ...
                                    <input type="text" required value={sec.name} onChange={(e) => handleSectionChange(nIdx, idx, 'name', e.target.value)} placeholder="e.g. A"
                                  ...
                                  <div className="w-full md:w-[16%]">
                                    ...
                                    <div className="custom-select-wrapper">
                                      <select value={sec.timeOption || 'select'} onChange={(e) => { const opt = e.target.value; handleSectionChange(nIdx, idx, 'timeOption', opt); if (opt !== 'select' && opt !== 'custom') handleSectionChange(nIdx, idx, 'startTime', ''); }}
                                  ...
                                  {sec.timeOption === 'custom' ? (
                                    <div className="w-full md:w-[36%]">
                                      ...
                                      <input type="text" value={sec.startTime || ''} onChange={(e) => handleSectionChange(nIdx, idx, 'startTime', e.target.value)} placeholder="e.g. 11:30 AM (Tentative)"
                                        className="w-full h-9 px-3 py-1.5 border border-hairline rounded-sm text-xs bg-canvas text-ink focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary hover:border-hairline-strong transition-all duration-150" />
                                    </div>
                                  ...
                                  <div className="w-full md:w-[20%]">
                                    ...
                                    <div className="custom-select-wrapper">
                                      <select value={sec.mode} disabled={notice.makeupStatus === 'online'} onChange={(e) => handleSectionChange(nIdx, idx, 'mode', e.target.value)}
                                  ...
                                  {sec.mode === 'Offline' && (
                                    <div className="w-full md:w-[18%]">
                                      ...
                                      <input type="text" value={sec.room} onChange={(e) => handleSectionChange(nIdx, idx, 'room', e.target.value)} placeholder="e.g. 611"
                                        className="w-full h-9 px-3 py-1.5 border border-hairline rounded-sm text-xs bg-canvas text-ink focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary hover:border-hairline-strong transition-all duration-150" />
                                    </div>
                                  )}
                                </div>
                        ...
                        {notice.category === 'class_cancel' && (
                          <div className="bg-canvas-soft border border-hairline rounded-sm p-4 space-y-3">
                            ...
                            <div className="custom-select-wrapper">
                              <select value={notice.makeupStatus} onChange={(e) => { const statusVal = e.target.value; handleNoticeFieldChange(nIdx, 'makeupStatus', statusVal); if (statusVal === 'online') setNotices(prev => { const u = [...prev]; u[nIdx].sections = u[nIdx].sections.map(sec => ({ ...sec, mode: 'Online', room: '' })); return u; }); }}
                                className="custom-select block w-full pl-3 pr-10 h-9 py-1.5 border border-hairline rounded-sm bg-canvas focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary text-sm text-ink hover:border-hairline-strong transition-all duration-150">
                            ...
                            {notice.makeupStatus === 'custom' && (
                              <div className="mt-2.5">
                                ...
                                <input type="text" required value={notice.customMakeupText} onChange={(e) => handleNoticeFieldChange(nIdx, 'customMakeupText', e.target.value)} placeholder="e.g. Makeup class on Friday at 3:00 PM in Room 602."
                                  className="appearance-none block w-full h-9 px-3 py-1.5 border border-hairline rounded-sm shadow-sm placeholder-ink-faint focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary text-sm text-ink hover:border-hairline-strong transition-all duration-150" />
                              </div>
                            )}
                          </div>
                        ...
                        {showTopics && (
                          <div className="bg-canvas-soft border border-hairline rounded-sm p-4 space-y-4">
                            ...
                            <div className="flex gap-2">
                              <input type="text" value={notice.currentTopic || ''} onChange={(e) => handleNoticeFieldChange(nIdx, 'currentTopic', e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); addTopic(nIdx); } }} placeholder={notice.category === 'syllabus' ? 'Type syllabus detail...' : notice.category === 'suggestion' ? 'Type suggestion...' : 'Type topic and press Enter...'}
                            ...
                            {notice.topics.length > 0 && (
                              <div className="space-y-1.5 max-h-[200px] overflow-y-auto p-1.5 border border-hairline rounded-sm bg-canvas">
                                {notice.topics.map((t, i) => (
                                  <div key={i} draggable onDragStart={(e) => handleTopicDragStart(e, nIdx, i)} onDragOver={(e) => e.preventDefault()} onDrop={(e) => handleTopicDrop(e, nIdx, i)}
                        ...
                        <div className="bg-canvas-soft border border-hairline rounded-sm p-4 space-y-4">
                          ...
                          <div className="flex gap-2">
                            <select value={notice.noteType || 'note'} onChange={(e) => handleNoticeFieldChange(nIdx, 'noteType', e.target.value)}
                            ...
                            <input type="text" value={notice.currentNote || ''} onChange={(e) => handleNoticeFieldChange(nIdx, 'currentNote', e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); addNote(nIdx); } }} placeholder="Add cover page / submit slides link..."
                          ...
                          {notice.notes.length > 0 && (
                            <div className="space-y-1.5 max-h-[200px] overflow-y-auto p-1.5 border border-hairline rounded-sm bg-canvas">
                              {notice.notes.map((n, i) => {
                                ...
                                return (
                                  <div key={i} draggable onDragStart={(e) => handleNoteDragStart(e, nIdx, i)} onDragOver={(e) => e.preventDefault()} onDrop={(e) => handleNoteDrop(e, nIdx, i)}
                                    className="flex items-center justify-between text-xs text-ink-secondary py-1.5 px-2 hover:bg-canvas-soft rounded-sm border border-transparent hover:border-hairline transition-all duration-150 cursor-move select-none">
                                    <span className="truncate flex items-center gap-1.5">
                                      <GripVertical className="w-3.5 h-3.5 text-ink-mute cursor-grab active:cursor-grabbing flex-shrink-0" />
                                      <DropdownMenu>
                                        <DropdownMenuTrigger asChild>
                                          <button type="button" draggable={false} onPointerDown={(e) => e.stopPropagation()} onMouseDown={(e) => e.stopPropagation()}
                                    ...
                                    <button type="button" draggable={false} onPointerDown={(e) => e.stopPropagation()} onMouseDown={(e) => e.stopPropagation()} onClick={() => removeNote(nIdx, i)}
              ...
              <button type="button" onClick={() => setNotices(prev => [...prev, createNewNoticeObj(prev.length)])}
              ...
              <div className="bg-canvas border border-hairline rounded-md p-4 space-y-2">
                ...
                <textarea value={closingText} onChange={(e) => setClosingText(e.target.value)} placeholder="e.g. Please be prepared and attend on time. Good luck! 🍀📖" rows={3}
