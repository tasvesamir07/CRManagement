...
import type { UploadedFile, Folder } from './types';
...
export default function LibraryModal({ show, onClose, onAttach, uploadedFiles, onPreview }: LibraryModalProps) {
  const [libFiles, setLibFiles] = useState<UploadedFile[]>([]);
  const [libLoading, setLibLoading] = useState(false);
  const [libSearch, setLibSearch] = useState('');
  const [libPage, setLibPage] = useState(1);
  const [libSelectedIds, setLibSelectedIds] = useState<number[]>([]);
  const [libCurrentFolderId, setLibCurrentFolderId] = useState<number | null>(null);
      ...
      <div className="bg-canvas border border-hairline rounded-lg shadow-xl max-w-2xl w-full flex flex-col max-h-[85vh]">
        ...
        <div className="p-4 border-b border-hairline">
          <input
            ...
            value={libSearch}
            onChange={(e: React.ChangeEvent<HTMLInputElement>) => { setLibSearch(e.target.value); setLibPage(1); }}
