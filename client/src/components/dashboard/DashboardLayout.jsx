...
import {
  ...
  LogOut, Menu, User, Shield, ClipboardList, FileUp,
  Sun, Moon, WifiOff
} from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-canvas-soft flex flex-col md:flex-row">
      ...
      <div className="md:hidden bg-canvas border-b border-hairline px-4 py-3 flex items-center justify-between sticky top-0 z-50">
        ...
        <div className="flex items-center gap-2">
          <button onClick={toggleTheme} className="p-2 text-ink-mute hover:text-ink rounded-sm hover:bg-canvas-soft transition-colors cursor-pointer" title={theme === 'light' ? 'Dark Mode' : 'Light Mode'}>
        ...
        {user?.role === 'admin' ? (
          <>
            ...
            <Link to="/profile" className={`flex flex-col items-center ${location.pathname === '/profile' ? 'text-primary' : 'text-ink-mute'}`}>
              <User className="w-5 h-5" /><span className="text-[10px] mt-1">Profile</span>
            </Link>
