...
const AdminDashboard = () => {
  ...
  const crUsers = users.filter((u: UserData) => u.role === 'cr').length;
  const activeUsers = users.filter((u: UserData) => u.is_active).length;
  const totalBroadcasts = announcements.length;
  const deliveredBroadcasts = announcements.filter((a: AnnouncementData) => a.status === 'sent' || a.status === 'partial').length;
  const scheduledBroadcasts = announcements.filter((a: AnnouncementData) => a.status === 'scheduled').length;
