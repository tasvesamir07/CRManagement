...
export default function ExpiryModal({ show, file, onClose, onUpdated }) {
  const [customExpiryDate, setCustomExpiryDate] = useState('');

  if (!show || !file) return null;

  const handleUpdateExpiry = async (newExpiresAt) => {
    try {
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="space-y-3">
            ...
            <div className="pt-2 border-t border-hairline">
              ...
              <input type="date" value={customExpiryDate} onChange={(e) => setCustomExpiryDate(e.target.value)} className="w-full px-3 py-2 text-sm border border-hairline rounded-sm bg-canvas text-ink focus:outline-none focus:border-primary transition-colors" />
            </div>
