...
import { Plus, Trash2, X, AlertCircle, Edit2, Palette } from 'lucide-react';
...
const ExamRoutineManager = () => {
  ...
  const [isCanvaMode, setIsCanvaMode] = useState(false);
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex gap-2 self-start sm:self-auto">
          {courses.length > 0 && (
            <>
              <button onClick={() => setIsCanvaMode(!isCanvaMode)}
                className={`flex items-center justify-center px-4 py-2 text-sm font-medium border rounded-sm transition-colors cursor-pointer shadow-sm ${
                  isCanvaMode
                    ? 'bg-canvas border-hairline text-ink hover:bg-canvas-soft'
                    : 'text-on-primary bg-primary border-primary hover:bg-primary-deep'
                }`}>
                <Palette className="w-4.5 h-4.5 mr-2" /> {isCanvaMode ? 'View Routine Table' : 'Canva Designer'}
              </button>
              {!isCanvaMode && (
