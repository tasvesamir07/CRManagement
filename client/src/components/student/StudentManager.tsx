      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-mono font-bold bg-primary/10 text-primary border border-primary/20 shadow-sm">
              {students.length} Total
            </span>
          </div>
          <p className="text-xs sm:text-sm text-ink-mute mt-1">Manage student records, batch information, and course enrollment.</p>
        </div>
        <div className="flex gap-3 self-start sm:self-auto">
          <button onClick={() => setShowBulkModal(true)}
            className="inline-flex items-center justify-center px-4 py-3 rounded-xl font-bold text-xs glass-card text-ink hover:bg-canvas-soft transition-all cursor-pointer">
            <Upload className="w-4 h-4 mr-2 text-indigo-400" /> Bulk Import
          </button>
          <button onClick={handleOpenAddForm}
            className="inline-flex items-center justify-center px-5 py-3 rounded-xl font-bold text-xs text-on-primary bg-gradient-to-r from-primary via-emerald-400 to-accent-cyan hover:scale-[1.02] active:scale-[0.98] shadow-lg shadow-primary/25 transition-all cursor-pointer">
