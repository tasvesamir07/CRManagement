  ...
  return (
    <div className="bg-surface rounded-lg border border-border p-4 animate-pulse">
      ...
      {Array.from({ length: lines }).map((_, i) => (
        <div key={i} className="h-3 bg-gray-200 rounded w-full mb-1.5" style={{ width: `${80 - i * 15}%` }} />
      ))}
      ...
      {Array.from({ length: rows }).map((_, row) => (
        <div key={row} className="flex gap-4 py-3 border-b border-border/50">
          {Array.from({ length: cols }).map((_, col) => (
            <div key={col} className="h-3 bg-gray-200 rounded flex-1" style={{ width: `${60 + col * 10}%` }} />
          ))}
        </div>
      ))}
    </div>
