...
/**
 * @openapi
 * /exam-routines/settings:
 *   get:
 *     tags: [Exam Routines]
 *     summary: Get persisted exam routine layout & design settings
 *   post:
 *     tags: [Exam Routines]
 *     summary: Save exam routine layout & design settings
 */
router.get('/settings', authMiddleware, async (req, res) => {
    try {
        const result = await db.query("SELECT value FROM system_settings WHERE key = $1", ['exam_routine_settings']);
